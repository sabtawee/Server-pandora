...
int guild_castledataloadack(int len, struct guild_castle *gc) {
	...
	if( ev < 0 ) { //No castles owned, invoke OnAgitInit as it is.
		...
		npc_event_doall( script_config.agit_init4_event_name );
	...
	for( i = 0; i < n; i++, gc++ ) {
		...
		if( c->guild_id ) {
			if( i != ev )
				...
				guild_npc_request_info(c->guild_id, event_name);
				snprintf( event_name, EVENT_NAME_LENGTH, "::%s", script_config.agit_init4_event_name );
				guild_npc_request_info(c->guild_id, event_name);
			}
		}
	}
...
bool guild_agit4_start(void){
	...
	npc_event_runall( script_config.agit_start4_event_name );
...
bool guild_agit4_end(void){
	...
	npc_event_runall( script_config.agit_end4_event_name );
