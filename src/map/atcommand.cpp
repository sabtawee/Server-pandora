...
ACMD_FUNC(agitstart4)
{
	...
	if( guild_agit4_start() ){
		clif_displaymessage(fd, "Battle Royale WAR has been initiated."); // "War of Emperium TE has been initiated."
		return 0;
	}else{
		clif_displaymessage(fd, "Battle Royale WAR is currently in progress"); // "War of Emperium TE is currently in progress."
		return -1;
	}
}
...
ACMD_FUNC(agitend4)
{
	...
	if( guild_agit4_end() ){
		clif_displaymessage(fd, "Battle Royale WAR has been ended.");// War of Emperium TE has been ended.
		return 0;
	}else{
		clif_displaymessage(fd, "Battle Royale WAR is currently not in progress.");// War of Emperium TE is currently not in progress.
		return -1;
	}
}
...
ACMD_FUNC(pettalk)
{
	...
	{// pet emotion processing
		const char* emo[] = {
			...
			"/singapore", "/brazil", "/fsh", "/spin", "/sigh", "/dum", "/crwd", "/desp","/india", "/love",
			"/russia", "-?-", "/mobile", "/mail",
	...
	for (k = 0; k < count; k++) {
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			if(item_data->type == IT_CARD && sd->sc.data[SC_CARDBOOST]){
				droprate += droprate * ((sd->sc.data[SC_CARDBOOST]->val1-100)/100);
			}		
...
/*==========================================
 * @npcinv spawn NPC for party invite
 * at party leader locate
 *------------------------------------------*/
ACMD_FUNC(npcpt)
{
	//---------------------
	...
	int map_x,map_y,type = BL_PC;
	int leader_party_cid;
	int ii = 0;
	char mapname[MAP_NAME_LENGTH];

	nullpo_retr(-1, sd);
	
	int script_check = static_cast<int>(mapreg_readreg(add_str("$enable_npcpt")));	
	
	if(!script_check){
		ShowError("NPCPT: Disabled, please re-check '$enable_npcpt' in npcpt.txt file script.\n");
		return 0;
	}
	...
	if( ( p = party_search(party_id) ) != NULL )
	{
		...
		party_name = "NULL";
		clif_displaymessage(fd, "ไม่สามารถใช้งานคำสั่งได้ เพราะคุณไม่ได้อยู่ในปาร์ตี้");
		return 0;
	}
	//---------------------
	...
	if (p)
	for(ii = 0; ii < MAX_PARTY && !p->party.member[ii].leader; ii++);
	...
	leader_party_cid = p->party.member[ii].char_id;

	if(sd->status.char_id != leader_party_cid){
		clif_displaymessage(fd, "ไม่สามารถใช้งานคำสั่งได้ เพราะคุณไม่ใช้หัวหน้าปาร์ตี้");
		return 0;
	}

	if(sd->state.npc_invparty == 1){
		clif_displaymessage(fd, "ไม่สามารถเรียก NPC Board Party ได้มากกว่า 1 ตัว");
		return 0;
	}
	
	int cost_amt = static_cast<int>(mapreg_readreg(add_str("$npcpt_zeny_cost")));
	
	// Try to pay for the refine
	if( pc_payzeny( sd, cost_amt, LOG_TYPE_COMMAND, NULL ) ){
		clif_displaymessage(fd, "คุณมี Zeny ไม่พอสำหรับใช้งานคำสั่งนี้");
		return 0;
	}	
	...
	int dir = 5;
	int spriteid, map_xs = -1, map_ys = -1, sourceid, npc_type, mapid, i;
	const char *sourcename = "og_board_party";
	const char *new_shown_name = "Board Party";

	char new_hidden_name[24] = "";
    std::sprintf(new_hidden_name, "%d", party_id);

	char cr_completed[60] = "";
	const char *cr_part1 = "[ ";
	const char *cr_part2 = " ]";
	strcat(cr_completed, cr_part1);
	strcat(cr_completed, party_name);
	strcat(cr_completed, cr_part2);

	//---------------------
	char ann_mes[300] = "";
	const char *anmes       = "ปาร์ตี้ ";
	char ann_mes_extra[300] = " ได้ปัก Board Party ณ แผนที่ ";
	const char *anmes_part1 = "[";
	const char *anmes_part1_1 = " [";
	const char *anmes_part2 = "]";
	const char *anmes_part2_1 = "]";
	const char *anmes_part3 = "|";
	strcat(ann_mes, anmes);
	strcat(ann_mes, anmes_part1);
	strcat(ann_mes, party_name);
	strcat(ann_mes, anmes_part2);
	strcat(ann_mes, ann_mes_extra);
	strcat(ann_mes, mapname);
	strcat(ann_mes, anmes_part1_1);

	char s_map_x[5] = "";
	char s_map_y[5] = "";
	std::sprintf(s_map_x, "%d", map_x);
	std::sprintf(s_map_y, "%d", map_y);

	strcat(ann_mes, s_map_x);
	strcat(ann_mes, anmes_part3);
	strcat(ann_mes, s_map_y);
	strcat(ann_mes, anmes_part2_1);

	clif_broadcast(bl, ann_mes, (int)strlen(ann_mes)+1, BC_BLUE, ALL_CLIENT);
	//---------------------

	char new_npc_name[24] = "";
	char chatroom_title[60] = "";

	strcat(chatroom_title, cr_completed);
	...
	strcat(new_npc_name, "#");
	strcat(new_npc_name, new_hidden_name);
	...
	nd_target->subtype = (enum npc_subtype)npc_type;
	...
	sd->state.npc_invparty = 1;

	chat_createnpcchat(nd_target, chatroom_title, 0, 1, 1, "", 0, 1, MAX_LEVEL);

	clif_displaymessage(fd, "ทำการสร้าง NPC สำหรับเชิญปาร์ตี้เสร็จสิ้น");
	return 0;

}

ACMD_FUNC(npcpt_reset)
{
	nullpo_retr(-1, sd);
	sd->state.npc_invparty = 0;
	return 0;
