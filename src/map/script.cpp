...
BUILDIN_FUNC(npcshopadditem)
{
	...
	if (nd->subtype == NPCTYPE_MARKETSHOP) {
		for (int n = 0, i = 3; n < amount; n++, i += offs) {
			...
			nd->u.shop.shop_item[j].maxbuy = script_getnum(st,i+3);
...
/*=========================================================================
 * guild_member_count(<map>,<Guild ID);
 *-------------------------------------------------------------------------*/
static int buildin_countgmember_sub(struct block_list *bl,va_list ap)
{
	struct map_session_data *sd = (TBL_PC *)bl;
...
BUILDIN_FUNC(guild_member_count)
{
	int count,m;
	int GID;
	const char *mapname;
	...
	count = map_foreachinmap(buildin_countgmember_sub, m, BL_PC,GID);	
	script_pushint(st, count);
	return SCRIPT_CMD_SUCCESS;
}
...
/*=========================================================================
 * party_member_count(<map>,<Guild ID);
 *-------------------------------------------------------------------------*/
static int buildin_countpmember_sub(struct block_list *bl,va_list ap)
{
	struct map_session_data *sd = (TBL_PC *)bl;
	int count = 0;
	int PID = va_arg(ap, int);
	if(sd->status.party_id == PID)
		if(!pc_isdead(sd))
			count++;
	return count;
}

BUILDIN_FUNC(party_member_count)
{
	int count,m;
	int PID;
	const char *mapname;
	...
	count = map_foreachinmap(buildin_countpmember_sub, m, BL_PC,PID);	
	script_pushint(st, count);
	return SCRIPT_CMD_SUCCESS;
}
...
// <--- [zBuffer] List of unit control commands
	...
	BUILDIN_DEF(party_member_count,"si"),
	BUILDIN_DEF(duplicateremove, "?"),
	BUILDIN_DEF(guild_member_count,"si"),
