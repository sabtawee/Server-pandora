...
void clif_dropflooritem( struct flooritem_data* fitem, bool canShowEffect ){
	...
	if( canShowEffect ){
		...
		}/*else{	// APACHE ITEM TYPE DROPEFFECT
			p.showdropeffect = 0;	// APACHE ITEM TYPE DROPEFFECT
			p.dropeffectmode = 0;	// APACHE ITEM TYPE DROPEFFECT
		}*/	// APACHE ITEM TYPE DROPEFFECT
	}else{
		p.showdropeffect = 0;
		p.dropeffectmode = 0;
	...
	switch( bl->type ){
		case BL_PC: {
			...
			if (battle_config.show_rank > 0) {
				safestrncpy( packet.party_name, sd->status.reputation, NAME_LENGTH );
			} else if( p && ( sd->guild || battle_config.display_party_name ) ){ // do not display party unless the player is also in a guild
